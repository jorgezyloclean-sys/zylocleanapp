...
-- Administración carga las plantillas en español; el personal (y el portal) las ven en su
-- idioma si hay traducción cargada. Estructura:
--   traducciones = { "en": { "nombre": "...", "tareas": ["...", ...] }, "is": { ... } }
-- Las tareas traducidas van alineadas por índice con `tareas`; una traducción vacía cae al español.

alter table public.checklists
  add column if not exists traducciones jsonb not null default '{}'::jsonb;

comment on column public.checklists.traducciones is
  'Traducciones por idioma ({en,is} → {nombre, tareas[]}), alineadas por índice con tareas.';

-- El portal también necesita las traducciones (el cliente elige idioma).
